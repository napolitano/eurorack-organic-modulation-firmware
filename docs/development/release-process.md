...
10. run Classic, Organic, Generative and Ambient native tests/coverage/sanitizers where present in the tagged source, both bootloader builds for each selected bank, resource-budget checks and the corresponding timing-probe builds;
...
Release packaging follows the capabilities of the tagged source. Current multi-bank tags package **twelve independently flashable bank/bootloader variants**, yielding **24 firmware files** in total because every variant is published as both HEX and ELF: Classic, Organic, Generative, Ambient, Electronica and Percussion, each for the new and old Arduino Nano bootloader. They are accompanied by **twelve BUILD-INFO provenance files**. Historical Classic-only tags package the two Classic bootloader variants only; intermediate tags package only the banks actually present in their PlatformIO environments. Filenames encode bank, bootloader and release version, for example `fm-drift-generative-nano-new-bootloader.X.Y.Z.hex`. `FIRMWARE-ARTIFACTS.X.Y.Z.md` is generated with the release and maps each filename to its bank, bootloader and rear-DIP algorithm slots. A separate `BUILD-INFO-*.X.Y.Z.txt` accompanies every generated bank/bootloader variant and records its exact PlatformIO environment and toolchain provenance.
