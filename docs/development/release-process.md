...
Release packaging follows the capabilities of the tagged source. Release `0.2.0` packages **twelve independently flashable bank/bootloader variants**, yielding **24 firmware files** because its six banks are each published for both Nano bootloaders as HEX and ELF, accompanied by **twelve BUILD-INFO provenance files**. The current Unreleased source additionally contains Dubstep/Bass; a future tag that includes it will therefore package fourteen bank/bootloader variants, **28 firmware files** and **14 BUILD-INFO files**. Historical Classic-only tags package the two Classic bootloader variants only; intermediate tags package only the banks actually present in their PlatformIO environments. Filenames encode bank, bootloader and release version, for example `fm-drift-generative-nano-new-bootloader.X.Y.Z.hex`. `FIRMWARE-ARTIFACTS.X.Y.Z.md` is generated with the release and maps each filename to its bank, bootloader and rear-DIP algorithm slots. A separate `BUILD-INFO-*.X.Y.Z.txt` accompanies every generated bank/bootloader variant and records its exact PlatformIO environment and toolchain provenance.
