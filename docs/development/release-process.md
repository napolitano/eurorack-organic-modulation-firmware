...
10. run Classic and Organic native tests/coverage/sanitizers, both bootloader builds for each bank, resource-budget checks and both timing-probe builds;
...
Release packaging follows the capabilities of the tagged source. Dual-bank tags package **four independently flashable firmware images** as both HEX and ELF: Classic and Organic, each for the new and old Arduino Nano bootloader. Historical Classic-only tags package the two Classic bootloader variants only. Filenames encode bank, bootloader and release version, for example `fm-drift-organic-nano-new-bootloader.X.Y.Z.hex`. `FIRMWARE-ARTIFACTS.X.Y.Z.md` is generated with the release and maps each filename to its bank, bootloader and rear-DIP algorithm slots. A separate `BUILD-INFO-*.X.Y.Z.txt` accompanies every generated bank/bootloader variant and records its exact PlatformIO environment and toolchain provenance.
