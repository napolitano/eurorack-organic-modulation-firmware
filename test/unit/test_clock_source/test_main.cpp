...
fmd::clock::ClockUpdate acquireClock(
    fmd::clock::ClockSource& source,
...
void test_clock_uses_speed_knob_increment_until_two_valid_edges() {
  ...
  TEST_ASSERT_EQUAL_UINT32(
      fmd::clock::quarterIncrementFromPeriodSamples(100UL),
...
void test_clock_hysteresis_requires_low_release_before_next_rising_edge() {
  ...
  source.update(fmd::clock::kClockHighThresholdAdc, kInternal);
  ...
  source.update(fmd::clock::kClockLowThresholdAdc, kInternal);
  update = source.update(600U, kInternal);
  TEST_ASSERT_TRUE(update.externalActive);
  TEST_ASSERT_TRUE(update.externalAcquired);
}
...
void test_external_clock_times_out_after_two_and_a_half_periods() {
  ...
  const uint32_t timeout = fmd::clock::clockTimeoutSamples(100UL);
...
void test_external_period_conversion_matches_scheduler_phase_contract() {
  TEST_ASSERT_EQUAL_UINT32(6871948UL,
                           fmd::clock::quarterIncrementFromPeriodSamples(625UL));
  TEST_ASSERT_EQUAL_UINT32(250UL, fmd::clock::clockTimeoutSamples(100UL));
}
...
void test_clock_period_helpers_clamp_below_and_above_supported_range() {
  TEST_ASSERT_EQUAL_UINT32(
      fmd::clock::quarterIncrementFromPeriodSamples(
          fmd::clock::kClockMinimumPeriodSamples),
      fmd::clock::quarterIncrementFromPeriodSamples(0U));
  TEST_ASSERT_EQUAL_UINT32(
      fmd::clock::quarterIncrementFromPeriodSamples(
          fmd::clock::kClockMaximumPeriodSamples),
      fmd::clock::quarterIncrementFromPeriodSamples(UINT32_MAX));
  TEST_ASSERT_EQUAL_UINT32(
      fmd::clock::clockTimeoutSamples(
          fmd::clock::kClockMinimumPeriodSamples),
      fmd::clock::clockTimeoutSamples(0U));
  TEST_ASSERT_EQUAL_UINT32(
      fmd::clock::clockTimeoutSamples(
          fmd::clock::kClockMaximumPeriodSamples),
      fmd::clock::clockTimeoutSamples(UINT32_MAX));
}
