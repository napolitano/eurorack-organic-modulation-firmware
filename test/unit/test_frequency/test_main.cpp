...
void test_delta_time_has_mathematically_correct_minimum_frequency_endpoint() {
  ...
  TEST_ASSERT_EQUAL_UINT32(42950UL, fmd::getDeltaTime(tables, 0U, 0U, 0));
}
...
void test_delta_time_is_monotonic_for_knob() {
  ...
  for (uint16_t knob = 0U; knob <= 1023U; ++knob) {
    const uint32_t current = fmd::getDeltaTime(tables, knob, 0U, 0);
...
void test_delta_time_is_monotonic_for_cv() {
  ...
  for (uint16_t cv = 0U; cv <= 1023U; ++cv) {
    const uint32_t current = fmd::getDeltaTime(tables, 0U, cv, 0);
    TEST_ASSERT_GREATER_OR_EQUAL_UINT32(previous, current);
    previous = current;
  }
}
...
void test_positive_and_negative_offset_move_frequency_in_expected_direction() {
  MemoryReferenceTables tables;
  const uint32_t center = fmd::getDeltaTime(tables, 512U, 256U, 0);
  const uint32_t positive = fmd::getDeltaTime(tables, 512U, 256U, 8192);
  const uint32_t negative = fmd::getDeltaTime(tables, 512U, 256U, -8192);
...
void test_delta_time_clamps_out_of_range_adc_inputs_to_hardware_domain() {
  MemoryReferenceTables tables;
  TEST_ASSERT_EQUAL_UINT32(fmd::getDeltaTime(tables, 1023U, 1023U, 0),
                           fmd::getDeltaTime(tables, 0xFFFFU, 0xFFFFU, 0));
  TEST_ASSERT_EQUAL_UINT32(fmd::getDeltaTime(tables, 1023U, 0U, 0),
                           fmd::getDeltaTime(tables, 0xFFFFU, 0U, 0));
  TEST_ASSERT_EQUAL_UINT32(fmd::getDeltaTime(tables, 0U, 1023U, 0),
                           fmd::getDeltaTime(tables, 0U, 0xFFFFU, 0));
}
