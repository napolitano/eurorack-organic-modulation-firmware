...
Each Drift mode has a dedicated suite. Classic remains the default compile-time bank; Organic, Generative, Ambient, Electronica, Percussion and Dubstep/Bass are exercised by their respective `native_organic*`, `native_generative*`, `native_ambient*`, `native_electronica*`, `native_percussion*` and `native_dubstep*` environments.
...
Release `0.3.0` contains **236 native cases across 39 suites**, but CI and release qualification do **not** rerun all 236 cases for every compile-time bank. The unfiltered `native` environment is the single complete regression pass. Additional non-Classic native runs are filtered to the bank-dependent engine/selection/runtime/property/system suites. Sanitizer runs remain split between shared/Classic and bank-specific qualification. Coverage is stricter: Classic/core, the shared `ClockSource`, and each extended bank are measured as separate semantic scopes so a component is counted only where it actually belongs. This keeps compile-time-bank verification intact without multiplying the complete suite seven times per qualification mode.
...
Coverage is split into three semantic scope families so the metric says something useful. The Classic report retains the established **95% line / 75% branch** floor for Classic plus an explicit allow-list of genuinely universal production core files. The shared `ClockSource` used by Percussion and Dubstep/Bass is measured independently at **97% line / 90% branch** aggregate, with the same **95% line / 80% branch** per-file floor used for extended code. The six extended banks — Organic, Generative, Ambient, Electronica, Percussion and Dubstep/Bass — are measured against **bank-owned production sources only** under `lib/fmd/src/domain/<bank>/`; each must reach at least **97% line / 90% branch coverage in aggregate**, and every individual bank-owned production file must independently reach **95% line / 80% branch coverage** when it contains branch-bearing code. A well-covered helper therefore cannot hide an algorithm wrapper with weak coverage.

The separation is intentional. Shared runtime, engine and fixed-point code is qualified by the Classic/core report and by the bank-aware integration/system suites. `ClockSource` is neither Classic code nor owned by either rhythm bank, so it has its own scope and dedicated `unit/test_clock_source` coverage run. Charging shared code to an unrelated bank, or charging it to both consumers, produces misleading percentages. Bank reports therefore answer the narrower question: *how thoroughly is this bank's own implementation exercised?*
