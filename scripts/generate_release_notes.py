...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--banks", default="classic,organic,generative,ambient,electronica,percussion,dubstep", help="Comma-separated banks included in the release")
...
def render_notes(*, text: str, tag: str, repository: str, server_url: str, previous_tag: str = "", banks: tuple[str, ...] = ("classic", "organic", "generative", "ambient", "electronica", "percussion", "dubstep")) -> str:
    ...
    if detail:
        output.extend(["\n".join(detail), ""])
    titled_banks = [bank.title() for bank in banks]
    if len(titled_banks) == 1:
        bank_names = titled_banks[0]
    elif len(titled_banks) == 2:
        bank_names = " and ".join(titled_banks)
    else:
        bank_names = ", ".join(titled_banks[:-1]) + " and " + titled_banks[-1]
...
def main() -> int:
    ...
    try:
        ...
        if not banks or any(bank not in ("classic", "organic", "generative", "ambient", "electronica", "percussion", "dubstep") for bank in banks):
