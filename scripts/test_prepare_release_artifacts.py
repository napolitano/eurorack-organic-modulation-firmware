...
def main() -> int:
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            Path("README-BANK-DUBSTEP.md").write_text("Dubstep", encoding="utf-8")

            build_root = root / "build"
            output_dir = root / "dist"
            output_dir.mkdir()
            banks = release.parse_banks("classic,organic,generative,ambient,electronica,percussion,dubstep")
            ...
            assert len(copied) == 28
            assert len(set(copied)) == 28
            ...
            assert "README-BANK-DUBSTEP.md" in docs

            manifest = release.write_firmware_manifest(output_dir, "1.2.3", banks, images)
            text = (output_dir / manifest).read_text(encoding="utf-8")
            assert all(name in text for name in ("Classic", "Organic", "Generative", "Ambient", "Electronica", "Percussion", "Dubstep"))
            ...
            assert "fm-drift-dubstep-nano-new-bootloader.1.2.3.hex" in text
            ...
            assert "BUILD-INFO-dubstep-nano-old.1.2.3.txt" in text
