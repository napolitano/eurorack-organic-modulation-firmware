...
ClockUpdate ClockSource::update(uint16_t speedCvAdc, uint32_t internalQuarterIncrement) {
  ...
  if (inputHigh_) {
    if (speedCvAdc <= kClockLowThresholdAdc) {
      inputHigh_ = false;
    }
  } else if (speedCvAdc >= kClockHighThresholdAdc) {
    inputHigh_ = true;
    risingEdge = true;
  }
  ...
  if (externalActive_ &&
      samplesSinceReferenceEdge_ > clockTimeoutSamples(lastPeriodSamples_)) {
    externalActive_ = false;
    haveReferenceEdge_ = false;
    samplesSinceReferenceEdge_ = 0U;
    externalLost = true;
  ...
  if (risingEdge) {
    if (haveReferenceEdge_) {
      ...
      // A too-fast edge is treated as a glitch and does not replace the last
      // reference edge. A too-slow source will already have timed out above.
    } else {
